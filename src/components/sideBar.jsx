...
function SideBar(){
    return (
      <div className="w-[230px] h-[750px]  border border-light-gray rounded-r-lg  ">
        ...
        <NavLink linkTitle={'Home'} icon={<GoHome/>}/>
        <NavLink linkTitle={'Movies'} icon={<BiCameraMovie/>}/>
        <NavLink linkTitle={'Tv Series'} icon={<MdOndemandVideo/>}/>
        <NavLink linkTitle={'Upcoming'} icon={<MdOutlineCalendarMonth/>}/>
          </div>
          ...
          <div className='w-[200px] h-[200px] mt-[40px] ml-[15px] flex flex-col justify-center items-center border border-rose rounded-lg bg-200'>
            <h className='text-[16px] ml-2 font-bold'>Play movie quizes and earn free tickets</h>
<p className='mt-2 ml-2'>50k people are playing now</p>
<MovieButton buttonText={'Start Playing'} buttonStyle={' w-[150px] h-[40px] mt-5 bg-light-red text-white text-[14px]'}/>
          </div>

          <MovieButton buttonText={'Log Out'} buttonIconLeft={<TbLogout/>} buttonStyle={' w-[120px] mt-5 text-gray bg-white text-[13px]  ml-[40px]'}/>
      </div>
    )
  }
